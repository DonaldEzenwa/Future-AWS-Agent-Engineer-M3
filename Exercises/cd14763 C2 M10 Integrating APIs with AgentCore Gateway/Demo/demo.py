"""
================================================================================
WanderBot — DEMO: AgentCore Gateway
================================================================================
"""
...
Use these tools to help customers with their flight bookings.
Present results clearly and ask clarifying questions when needed."""


@app.entrypoint
async def invoke(payload: dict, context=None) -> dict:
    """WanderBot - AgentCore Gateway """
    user_message = payload.get("message", "Hello!")

    logger.info("User: %s", user_message[:80])

    # Connect to the Gateway — no auth headers needed
    client = MCPClient(
        lambda: streamable_http_client(url=GATEWAY_ENDPOINT)
    )
    with client:
        # Discover all tools registered in the Gateway
        tools = client.list_tools_sync()
        logger.info("Tool names: %s", [t.tool_name for t in tools])  # add this
